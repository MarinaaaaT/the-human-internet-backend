...
**Capture pipeline** (added 2026-09-25; the app sends it while its `server_side_watermark` flag is on). A request with `X-Capture-Pipeline: server-watermark-v1` + `X-Photo-Id` carries the **raw capture**: the Lambda signs it as a `digitalCapture` (`POST /capture`), then burns the brand mark into that signed capture and signs the result with the capture as its C2PA **parentOf ingredient** (`POST /watermark`); the signed capture is stored at `photo-originals/{user_id}/{photo_id}.jpg` (service role, upsert) and the watermarked photo is returned with the pipeline header echoed — the app refuses a response without it. The watermark step is only believed if the Lambda answers `x-signing-route: watermark`; a Lambda that predates routing would otherwise have silently signed it as another capture, unwatermarked, so its absence is a 502 and the app retries. Without the pipeline header the body is signed as-is (`POST /capture`), which is what every build predating this sends. Two Lambda calls rather than one returning both images because a Function URL response is capped at 6MB. Deploy order: `supabase/sql/2026-09-25_server_watermark.sql`, then the Lambda, then this function, then the app flag.
...
- What it proves is the request's origin, not the pixels: a jailbroken device can still hook the capture path. On the capture pipeline the assertion covers the raw capture; user edits beyond the watermark are the follow-on work (Notion Tech Debt: "[Provenance 2–4/4]").
...
**No WebCrypto signature checks in `_shared/appAttest.ts` — both the certificate chain and the assertion are verified with `@noble/curves`.** Supabase's Edge runtime throws `NotSupportedError: Not implemented` from `crypto.subtle.verify` for ECDSA over a P-384 key with SHA-256, which is exactly how Apple's App Attest CA signs every credential certificate. A local Deno supports it, so the first version passed every test and then 500'd every real registration (2026-09-25). A test now stubs `crypto.subtle.verify` to throw and requires both checks to pass anyway; keep it.
